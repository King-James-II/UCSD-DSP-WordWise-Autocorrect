...
public abstract class Document {
	 ...
	 */
		protected int countSyllables(String word)
		{
			...
			for (int i = 0; i < word.length(); i ++) {
				...
				if (i == word.length()-1 && Character.toString(currChar).matches("[eE]") && !lastVowel && numSyllables > 0) {
					numSyllables--;
				}
				if (Character.toString(currChar).matches("[aeiouyAEIOUY]") && !lastVowel) {
					lastVowel = true;
					numSyllables++;
				}
				else if (!Character.toString(currChar).matches("[aeiouyAEIOUY]")){
					lastVowel = false;
				}
			}
	...
	protected boolean isVowel (char c) {
		return Character.toString(c).matches("[AEIOUYaeiouy]");
	}
